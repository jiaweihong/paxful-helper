const getTradeInfo = async () => {
  try {
    if (window.location.pathname.length == 18) {
      ...
      let numberOfSellerFeedbackPages = Math.ceil(
      ...
      for (response in sellerFeedbackResponses) {
        sellerFeedbackJsonPromises.push(
          sellerFeedbackResponses[response].json()
        );
      }
      ...
      let fromSellerParaText = document.createTextNode(
